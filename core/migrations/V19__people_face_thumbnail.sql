...
-- Join table for faces and people
--CREATE TABLE faces_x_people (
--        face_id      INTEGER PRIMARY KEY UNIQUE NOT NULL,
--        person_id    INTEGER NOT NULL,
--        is_confirmed BOOLEAN NOT NULL CHECK (is_confirmed IN (0, 1)) DEFAULT 0, -- person_id confirmed by user?
--        is_thumbnail BOOLEAN NOT NULL CHECK (is_confirmed IN (0, 1)) DEFAULT 0, -- is face for thumbnail. Should only be one!
...
-- Migrate
--INSERT INTO faces_x_people (face_id, person_id, is_confirmed, is_thumbnail)
--SELECT
--        face_id,
--        person_id,
--        is_confirmed,
--        p.thumbnail_path = f.thumbnail_path AS is_thumbnail
--FROM pictures_faces AS f
--INNER JOIN people AS p USING (person_id);

--ALTER TABLE pictures_faces DROP COLUMN person_id;
--ALTER TABLE pictures_faces DROP COLUMN is_confirmed;

--ALTER TABLE people DROP COLUMN thumbnail_path;
